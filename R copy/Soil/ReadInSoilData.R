...
plotMetaData <- read_csv(file = "data/soil/Three-D_PlotLevel_MetaData_2019.csv") %>% 
  ...
  mutate(soil_depth_cm = (soil_depth1 + soil_depth2 + soil_depth3 + soil_depth4) / 4,
         ...
         year = 2019) %>% 
  select(year, origSiteID:turfID, warming:Nlevel, date_slope:slope, aspect = exposure, date_depth, soil_depth_cm, remark)

#write_csv(plotMetaData, path = "data_cleaned/soil/Three-D_soil_depth_2019.csv")
